...
if not df_ouv.empty:
    ...
    mapa_colunas = {
        ...
        "DT_CONCLUSAO": "DATA_FIM",
        "DATA RESPOSTA": "DATA_FIM",
        # Métricas
        "DIAS PARA RESOLUÇÃO": "PRAZO",
        "TEMPO_RESOLUCAO": "PRAZO",
        "DIAS_RESOLUCAO": "PRAZO",
        "PRAZO_ATENDIMENTO": "PRAZO",
        # Dimensões
        "SITUAÇÃO": "STATUS",
        "SITUACAO": "STATUS",
        "STATUS": "STATUS",
        "UF DO MUNICÍPIO MANIFESTAÇÃO": "UF",
        "UF_MANIFESTACAO": "UF",
        "UF": "UF",
        "UF_MANIFESTANTE": "UF",
        "ASSUNTO": "ASSUNTO",
    ...
    if "DATA_FIM" in df_ouv.columns:
        df_ouv["DATA_FIM"] = pd.to_datetime(
            df_ouv["DATA_FIM"], dayfirst=True, errors="coerce"
        )
...
opcoes_ano = (
    sorted(list(df_ouv["ANO"].dropna().unique()), reverse=True)
    if "ANO" in df_ouv.columns
    else []
)
            ...
            children=[
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            html.Div(
                                className="custom-card p-3 bg-white rounded shadow-sm h-100",
                                children=[
                                    html.H6(
                                        "Volume x Tempo de Resposta (Mensal)",
                                        className="fw-bold text-secondary mb-3",
                                    ),
                                    dcc.Graph(
                                        id="fig-combo-temporal",
                                        style={"height": "350px"},
                                        config={"displayModeBar": False},
                                    ),
                                ],
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            html.Div(
                                className="custom-card p-3 bg-white rounded shadow-sm",
                                children=[
                                    html.H6(
                                        "Ranking de Órgãos (Volume)",
                                        className="fw-bold text-secondary mb-3",
                                    ),
                                    dcc.Graph(
                                        id="fig-pareto-orgao",
...
@callback(
    [
        Output("kpi-qlik-vol", "figure"),
        Output("kpi-qlik-tempo", "figure"),
        Output("kpi-qlik-sla", "figure"),
        Output("kpi-qlik-pendencia", "figure"),
        Output("fig-combo-temporal", "figure"),
        Output("fig-top-assuntos", "figure"),
    ...
    [
        Input("ouv-ano", "value"),
        Input("ouv-uf", "value"),
        Input("ouv-btn-download", "n_clicks"),
    ],
)
def update_ouvidoria(anos, ufs, n_clicks):
    ...
    if not dff.empty:
        if anos:
            dff = dff[dff["ANO"].isin(anos if isinstance(anos, list) else [anos])]
        if ufs:
            dff = (
                dff[dff["UF"].isin(ufs if isinstance(ufs, list) else [ufs])]
    ...
    def criar_kpi_qlik(valor_atual, titulo, sulfixo="", cor=COR_NEUTRA):
        ...
        fig.add_trace(
            go.Indicator(
                ...
                number={
                    "suffix": sulfixo,
                    "font": {"size": 40, "color": cor, "family": "Inter, sans-serif"},
                },
                title={
                    "text": titulo.upper(),
        ...
        fig.update_layout(
            paper_bgcolor="white", height=120, margin=dict(l=10, r=10, t=30, b=10)
        )
        return fig
    ...
    if "STATUS" in dff.columns:
        pendentes = len(
            dff[
                ~dff["STATUS"]
                .astype(str)
                .str.contains("Concluída|Respondida|Encerrada", case=False, na=False)
            ]
        )
    ...
    kpi2 = criar_kpi_qlik(
        ...
        "Tempo Médio",
        " dias",
        COR_ALERTA if tempo_medio > 20 else COR_SUCESSO,
    )
    kpi3 = criar_kpi_qlik(
        sla_pct, "SLA (< 30 dias)", "%", COR_SUCESSO if sla_pct >= 80 else COR_ALERTA
    )
    kpi4 = criar_kpi_qlik(
        pendentes, "Em Aberto", "", COR_ERRO if pendentes > 0 else COR_SUCESSO
    )
    ...
    if "DATA" in dff.columns:
        ...
        df_g = (
            dff.groupby(dff["DATA"].dt.to_period("M").astype(str))
            .agg(
                Volume=("DATA", "count"),
                Tempo=(
                    ("PRAZO", "mean")
                    if "PRAZO" in dff.columns
                    else ("DATA", lambda x: 0)
                ),
            )
            .reset_index()
        )
        ...
        fig_combo.add_trace(
            go.Bar(
                x=df_g["Mes"], y=df_g["Volume"], name="Volume", marker_color=COR_BARRAS
            )
        )
        ...
        fig_combo = go.Figure().add_annotation(
            text="Sem dados Temporais", showarrow=False
        )
    ...
    if "ASSUNTO" in dff.columns:
        ...
        df_ass["Assunto"] = df_ass["Assunto"].apply(
    ...
    dl = (
        dcc.send_data_frame(dff.to_csv, "monitoramento_ouvidoria.csv", index=False)
        if n_clicks
        else None
    )
